...
public class DoctorScheduleServiceImpl implements DoctorScheduleService {
	...
	public boolean isDoctorAvailable(Long doctorId, LocalDateTime requestedTime) {
		...
		String requestedDay = requestedTime.getDayOfWeek().toString();
		String formattedDay = requestedDay.substring(0, 1).toUpperCase() + requestedDay.substring(1).toLowerCase();

		if (!schedule.getAvailableDays().contains(formattedDay)) {
			return false;
		}

		LocalTime requested = requestedTime.toLocalTime();
		String[] timeSlots = schedule.getAvailableTime().split(",");

		for (String slot : timeSlots) {
			LocalTime slotTime = LocalTime.parse(slot.trim());
			if (requested.equals(slotTime)) {
				return true;
			}
		}
